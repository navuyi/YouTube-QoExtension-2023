...
const FormSchema: ObjectSchema<form> = object({
  subjectSex: string<'male' | 'female' | 'undisclosed'>().required(
    'Subject sex is required'
  ),
  subjectAge: number()
    .typeError('Subject age must be a number')
    .min(1, 'Subject age must be greater than 0')
    .required('Subject age is required'),
});

export const useExperimentStart = () => {
  const [postExperiment, { isLoading: postingExperiment, error }] =
