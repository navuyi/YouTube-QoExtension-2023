...
export class DebugDataMonitor {
  ...
  private analyze = async (): Promise<void> => {
    ...
    const chunk: object = {
      ...
      viewport:
        this.debugDataElements.viewportFrames.innerText.split('/')[0] || null,
      droppedFrames:
        this.debugDataElements.viewportFrames.innerText
          .split('/')[1]
          .split('dropped of')[0] || null,
      totalFrames:
        this.debugDataElements.viewportFrames.innerText
          .split('/')[1]
          .split('dropped of')[1] || null,
      currentResolution:
        this.debugDataElements.currentOptimalRes.innerText.split('/')[0] ||
        null,
      optimalResolution:
        this.debugDataElements.currentOptimalRes.innerText.split('/')[1] ||
        null,
      volume:
        this.debugDataElements.volumeNormalized.innerText.split('/')[0] || null,
      normalizedVolume:
        this.debugDataElements.volumeNormalized.innerText.split('/')[1] || null,
  ...
  private getDebugDataElements = () => {
    const container = document.querySelector(
      HtmlQueryElements.playerDebugDataContent
    ) as HTMLElement;
    ...
    this.debugDataElements = {
      ...
      connectionSpeed: content[8]
        .querySelector('span')
        ?.querySelectorAll('span')[1] as HTMLSpanElement,
      networkActivity: content[9]
        .querySelector('span')
        ?.querySelectorAll('span')[1] as HTMLSpanElement,
      bufferHealth: content[10]
        .querySelector('span')
        ?.querySelectorAll('span')[1] as HTMLSpanElement,
  ...
  private openDebugMenu = (): void => {
    ...
    const menuButtons = document.querySelectorAll(
      HtmlQueryElements.playerContextMenuButton
    );
